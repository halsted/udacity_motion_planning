...
double BehaviorPlannerFSM::get_look_ahead_distance(const State& ego_state) {
  ...
  // vf = 0 so d = -vi**2/(2*a) = vi_mag**2/(2*a_mag)
  
  auto look_ahead_distance = velocity_mag*velocity_mag/(2*accel_mag);  // <- Fix This
  
  // Decelerate at 1/2 * (P_MAX_ACCEL)  
  //auto look_ahead_distance = velocity_mag*velocity_mag/(P_MAX_ACCEL);  // <- Fix This
...
State BehaviorPlannerFSM::state_transition(const State& ego_state, State goal,
  ...
  if (_active_maneuver == FOLLOW_LANE) {
    ...
    if (is_goal_in_junction) {
      ...
      // Velocity should be 0 at stopping point
